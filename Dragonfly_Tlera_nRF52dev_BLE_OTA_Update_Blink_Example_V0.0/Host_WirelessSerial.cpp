...
void WirelessSerial::evaluateCommand()
{
  switch(cmdWirelessSerial[CURRENTPORT])
  {
   ...
   case SEND_SERIAL_DATA:                                                                                                 // CMD 20
     cmdWirelessSerial[BLE_UART] = 10;
     WirelessSerial::headSerialRequest(BLE_UART, 0, 4*(UPDATE_SIZE));
     for(uint8_t i=0; i<(UPDATE_SIZE-1); i++)
     {
       WirelessSerial::serializefloat(data_update[i]);
     }
     WirelessSerial::tailSerialRequest(BLE_UART);
     break;
   default:
     break;
  }
...
{
  CURRENTPORT = BLE_UART;
  cmdWirelessSerial[BLE_UART] = 14;
  WirelessSerial::headSerialRequest(BLE_UART, 0, 1);
