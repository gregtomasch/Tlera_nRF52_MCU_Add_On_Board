...
uint8_t                          ack_nak_byte;                                                                      // Flow control byte; pending = 2, OTA update request ACK = 4, data block ACK = 1, data block resend = 3, NAK = 0
uint8_t                          serial_byte;
File                             file;
...
void flashFW()
{
  ...
  } else
  {
    ...
    for(uint16_t i=0; i<OTA_Update_Blocks; i++)
    {
      ...
      Serial.println(short_unsigned_byte[0]);
      OTA_bytes[1] = short_unsigned_byte[1];
      Serial.println(short_unsigned_byte[1]);
