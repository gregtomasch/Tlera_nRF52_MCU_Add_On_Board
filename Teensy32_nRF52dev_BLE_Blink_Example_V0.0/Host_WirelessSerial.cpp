...
void WirelessSerial::serialCom()
{
  ...
  for(n=0; n < UART_NUMBER; n++)                                                                                          // General function to handle multiple active ports. Limited to UART1 - UART3 for the IU Butterfly-based board
  {
    ...
    while (WirelessSerial::SerialAvailable(CURRENTPORT))
    {
      ...
      //Serial.write(c);                                                                                                    // Debug diagnostic to monitor incoming OTA request/OTA data block messages; uncomment to activate
...
void WirelessSerial::evaluateCommand()
{
  switch(cmdWirelessSerial[CURRENTPORT])
  {
   ...
   case INITIATE_OTA_UPDATE:                                                                                              // CMD 12
     ...
     // Placeholder for Teensy 3.X OTA update code; STM32L4 version functional
   ...
   case SEND_SERIAL_DATA:                                                                                                 // CMD 20
     cmdWirelessSerial[TEENSY_BLE_UART] = 10;
     WirelessSerial::headSerialRequest(TEENSY_BLE_UART, 0, 4*(UPDATE_SIZE));
     for(uint8_t i=0; i<(UPDATE_SIZE-1); i++)
     {
       WirelessSerial::serializefloat(data_update[i]);
     }
     WirelessSerial::tailSerialRequest(TEENSY_BLE_UART);
     break;
   default:
     break;
  }
...
{
  CURRENTPORT = TEENSY_BLE_UART;
  cmdWirelessSerial[TEENSY_BLE_UART] = 14;
  WirelessSerial::headSerialRequest(TEENSY_BLE_UART, 0, 1);
